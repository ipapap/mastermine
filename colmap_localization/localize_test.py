...
def localize_image(image_path,frames,frames_descriptors,encoder,top_k=1):
    ...
    img = Image.open(IMG_PATH_QUERY + images[image_id ]).convert('RGB')
    ...
    descriptor=encoder.model(encoder.preprocess_image(IMG_PATH_QUERY+images[image_id]).cuda()).detach().cpu()
...
for image_id in range(0,len(images)+0):
    ...
    # im=Image.open(IMG_PATH_QUERY+images[image_id])
    ...
    trans_error=np.linalg.norm(np.asarray([x,y,alt])-np.asarray([gt_x,gt_y,gt_alt])),'  Num of Inliers:',pose['num_inliers']
    r_errors.append(rot_error)
    t_errors.append(t_errors)
