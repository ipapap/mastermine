    ...
    def __init__(self,
        ...
        if 'swsl' in model_name or 'ssl' in model_name:
            ...
            if 'resnext50' in model_name:
                ...
                # if you want to load gsv-cities weights
                # checkpoint = torch.load('weights/resnet50_ConvAP_2048_2x2.ckpt', map_location='cpu')
                # adjusted_state_dict = {k.replace('backbone.model.', ''): v for k, v in checkpoint.items()}
                # # Load the state dict into your model and capture missing keys
