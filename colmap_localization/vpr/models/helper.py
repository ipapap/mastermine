...
def get_backbone(backbone_arch='resnet50',
    ...
    """
    if 'myresnet' in backbone_arch.lower():
        return backbones.myResNet(backbone_arch, pretrained, layers_to_freeze, layers_to_crop)
...
def get_aggregator(agg_arch='ConvAP', agg_config={}):
    ...
    elif 'equigem' in agg_arch.lower():
        if agg_config == {}:
            agg_config['p'] = 3
        else:
            assert 'p' in agg_config
        return aggregators.EquivariantGeMPool(**agg_config)
    ...
    elif 'myagg' in agg_arch.lower():
        assert 'in_channels' in agg_config
        return aggregators.myAgg(**agg_config)
